...
class tEnvoy {
  ...
  genKeys(args) {
	return new Promise(async (resolve, reject) => {
		...
		let privateArmored = openpgpkey.privateKeyArmored.replace("Version: OpenPGP.js v4.10.10", "Version: " + this.version).replace("Comment: https://openpgpjs.org", "Comment: https://togatech.org/ (Key generated by TogaTech tEnvoy)");
		let publicArmored = openpgpkey.publicKeyArmored.replace("Version: OpenPGP.js v4.10.10", "Version: " + this.version).replace("Comment: https://openpgpjs.org", "Comment: https://togatech.org/ (Key generated by TogaTech tEnvoy)");
		if(args.password == null) {
			...
			let encryptedPrivateKey = await openpgp.encrypt({
				message: await openpgp.message.fromText(privateArmored),
				passwords: [args.password]
			}).catch((err) => {
				reject(err);
			});
			let encryptedPublicKey = await openpgp.encrypt({
				message: await openpgp.message.fromText(publicArmored),
				passwords: [args.password]
			}).catch((err) => {
				reject(err);
			});
...
class tEnvoyKey {
	...
	getPrivate(password) {
		return new Promise(async (resolve, reject) => {
			...
			let openpgpkey = await openpgp.key.readArmored(privateKeyArmored).catch((err) => {
				reject(err);
			});
			...
			if(assertion.proceed) {
				if(this.#type == "private") {
					...
					if(this.#locked) {
						...
						let openpgpkey = await openpgp.key.readArmored(publicKeyArmored).catch((err) => {
							reject(err);
						});
						resolve(openpgpkey.keys[0]);
					}
	...
	setPublicArmored(keyArmored, password = null) {
		if(keyArmored == null) {
			...
			return new Promise(async (resolve, reject) => {
				let openpgpkey = await openpgp.key.readArmored(keyArmored).catch((err) => {
					reject(err);
				});
